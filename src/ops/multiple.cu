...
template<typename T>
__global__ void element_wise_mul128_backward_kernel(
...
Tensor mul(const Tensor& A,const Tensor& B){
    ...
    DISPATCH_DTYPE_AND_PACK(dtype,scalar_t,vec_t,{
        ...
        element_wise_mul128_kernel<scalar_t><<<blocks,threads>>>(
    ...
    if(out.storage_->requires_grad_){
        ...
        DISPATCH_DTYPE_AND_PACK(dtype,scalar_t,vec_t,{
            ...
            int threads = 256;
            int blocks = CEIL_DIV(A.storage_->size_,pack*threads);
            element_wise_mul128_backward_kernel<scalar_t><<<blocks,threads>>>(
